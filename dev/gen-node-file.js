...
function parseConfigtxYaml(path) {
  ...
  const defaultParams = JSON.parse(
    fs.readFileSync(defaultParamsFile).toString()
  );
  ...
  configTxObj.Organizations.forEach((org) => {
    if (!org.AnchorPeers) {
      ...
      orgJSON.pem = getBase64TLSCert(
        path + "/" + org.MSPDir + "/tlscacerts/tlsca." + domain + "-cert.pem"
      );
      ...
      const orgPem = getBase64TLSCert(
        path + "/" + org.MSPDir + "/tlscacerts/ca.crt"
      );
      let orgDefaultParams = defaultParams[org.Name];
      if (!orgDefaultParams) {
        throw new Error("No default parameters found for org " + org.Name);
      }
      orgDefaultParams.peer_ports.forEach((peer_port) => {
        const peerNames = Object.keys(peer_port);
        if (peerNames.length != 1) {
          throw new Error(
            "Expected exactly one peer name in structure for " +
              domain +
              ", found " +
              peerNames.length
          );
